...
define Build/Prepare
	...
	$(if $(QUILT),,( cd $(PKG_BUILD_DIR); $(STAGING_DIR)/opt/bin/phpize7 ))
endef
...
CONFIGURE_ARGS+= \
	--with-php-config=$(STAGING_DIR)/opt/bin/php7-config
...
define PHP7PECLPackage
  ...
  define Package/php7-pecl-$(1)/install
    ...
    ifeq ($(5),zend)
	echo "zend_extension=/opt/lib/php/$(subst -,_,$(1)).so" > $$(1)/opt/etc/php7/$(if $(4),$(4),20)_$(subst -,_,$(1)).ini
    else
